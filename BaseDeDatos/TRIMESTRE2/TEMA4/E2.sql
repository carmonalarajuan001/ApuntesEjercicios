...
SELECT AVG(goles_local + goles_visitante) AS promedio_goles_partido
FROM partidos
WHERE id_equipo_local = 1
   OR id_equipo_visitante = 1;
...
SELECT e.nombre_equipo,
       SUM(p.goles_local) AS total_goles
FROM equipos e
...
SELECT id_equipo_local,
       COUNT(*) AS victorias_local,
       AVG(goles_local) AS promedio_goles_local
FROM partidos
WHERE goles_local > goles_visitante
GROUP BY id_equipo_local
ORDER BY victorias_local DESC, promedio_goles_local DESC;

--********************************************************--
-- AVG	Calcula el promedio	Edad media
-- COUNT	Cuenta registros	Número de jugadores
-- SUM	Suma valores	Total de goles
--********************************************************--
