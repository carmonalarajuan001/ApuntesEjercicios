...
-- 1. Muéstrame las Personas y sus coches (solo los que tengan algun coche)
    SELECT p.nombre, p.apellido, c.marca, c.modelo
    FROM personas p
    JOIN coches c ON p.id = c.id_dueño;
-- 2. Coches con información de dueños como su nombre y su provincia
    SELECT c.marca, c.modelo, p.nombre, p.apellido, p.provincia
    FROM coches c
    JOIN personas p ON c.id_dueño = p.id;
-- 3. Personas que tienen coches y su estado civil
    SELECT DISTINCT p.nombre, p.apellido, p.estado_civil
    FROM personas p
    JOIN coches c ON p.id = c.id_dueño;
-- 4. Coches de personas ricas (niv socioeconomico alto)
    SELECT c.marca, c.modelo, p.nombre, p.apellido
    FROM coches c
    JOIN personas p ON c.id_dueño = p.id
    WHERE p.nivel_socioeconomico = 'Alto';
-- 5. Ocupaciones (distintas) de dueños de coches mayores de 30 años
    SELECT DISTINCT p.ocupacion
    FROM personas p
    JOIN coches c ON p.id = c.id_dueño
    WHERE TIMESTAMPDIFF(YEAR, p.fecha_nacimiento, CURDATE()) > 30;
-- Nivel 3: JOINS con Agregaciones
-- 11. Número de coches por persona
    SELECT p.nombre, p.apellido, COUNT(c.id) AS numero_coches
    FROM personas p
    LEFT JOIN coches c ON p.id = c.id_dueño
    GROUP BY p.id;
-- 12. Marca más común entre personas de cada provincia
    SELECT p.provincia, c.marca, COUNT(*) AS cantidad
    FROM personas p
    JOIN coches c ON p.id = c.id_dueño
    GROUP BY p.provincia, c.marca
    ORDER BY cantidad DESC;
-- 13. Promedio de edad de dueños por marca de coche
    SELECT c.marca, AVG(TIMESTAMPDIFF(YEAR, p.fecha_nacimiento, CURDATE())) AS promedio_edad
    FROM coches c
    JOIN personas p ON c.id_dueño = p.id
    GROUP BY c.marca;
-- 14. Total de ingresos de dueños por color de coche
    SELECT c.color, SUM(p.ingresos_anuales) AS total_ingresos
    FROM coches c
    JOIN personas p ON c.id_dueño = p.id
    GROUP BY c.color;
-- 15. Municipios con más coches
SELECT p.municipio, COUNT(c.id) AS total_coches FROM personas p JOIN coches c ON p.id =c.id_dueño GROUP BY municipio ORDER BY total_coches DESC;
-- Nivel 2: LEFT JOIN
...
-- 8. Provincias con y sin representación de coches
    SELECT p.provincia, COUNT(c.id) AS total_coches
    FROM personas p
    LEFT JOIN coches c ON p.id = c.id_dueño
    GROUP BY p.provincia;
-- 9. Nivel de estudios de las personas sin coche
    SELECT p.nivel_estudios, COUNT(*) AS cantidad
    FROM personas p
    LEFT JOIN coches c ON p.id = c.id_dueño
    WHERE c.id IS NULL
    GROUP BY p.nivel_estudios;
-- 10. Personas jubiladas con sus vehículos
    SELECT p.nombre, p.apellido, c.marca, c.modelo
    FROM personas p
    LEFT JOIN coches c ON p.id = c.id_dueño
    WHERE p.ocupacion = 'Jubilado';
-- Nivel 4: JOINS Complejos
-- 16. Padres e hijos y sus coches
    SELECT p.nombre AS nombre_padre, p.apellido AS apellido_padre, h.nombre AS nombre_hijo, h.apellido AS apellido_hijo, c.marca, c.modelo
    FROM personas p
    JOIN personas h ON p.id = h.padre OR p.id = h.madre
    JOIN coches c ON h.id = c.id_dueño;
