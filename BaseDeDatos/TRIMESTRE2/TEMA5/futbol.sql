...
SELECT e.nombre AS equipo, j.nombre AS jugador, j.apellidos AS apellidos_jugador
FROM equipos e
...
SELECT e.nombre AS equipo, AVG(j.salario_anual) AS salario_promedio
FROM equipos e
LEFT JOIN jugadores j ON e.id_equipo = j.id_equipo
...
FROM equipos e
LEFT JOIN jugadores j ON e.id_equipo = j.id_equipo
WHERE j.id_equipo IS NULL;

-- 6. Jugadores por posición en cada equipo
SELECT e.nombre AS equipo, j.posicion, COUNT(*) AS num_jugadores
FROM equipos e
LEFT JOIN jugadores j ON e.id_equipo = j.id_equipo
GROUP BY e.id_equipo, j.posicion;
